...
public class FixMovedTooFast {
	...
	double redirectVelocity(double original, PlayerMoveC2SPacket packet) {
		if (MainRenderManager.getCurrentTunnel() == null) return original;
		...
		return MainRenderManager.getCurrentTunnel().rayToIllusionSpace(this.player.getPos(), new Vec3d(moveX, moveY, moveZ)).lengthSquared();
	}
}
